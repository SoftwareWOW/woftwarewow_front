...
/** Hide non-software categories for the web applications service page only. */
...
const softwareTechCategories = techCategories.filter((cat) => !HIDDEN_CATEGORY_IDS.has(cat.id))

/** Layout: about/TechStack — filtered for software development (does not change about TechStack). */
const TechStack = () => {
  const [activeId, setActiveId] = useState(softwareTechCategories[0].id)
  const active = softwareTechCategories.find((c) => c.id === activeId) ?? softwareTechCategories[0]
  const tabsId = useId()
    ...
    >
      ...
      <div className="container relative">
        ...
        <LayoutGroup id={tabsId}>
          ...
          >
            {softwareTechCategories.map((cat) => {
