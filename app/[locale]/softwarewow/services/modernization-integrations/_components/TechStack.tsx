...
/** Keep integration-focused categories for this service page only. */
...
const modernizationTechCategories = techCategories.filter((cat) => !HIDDEN_CATEGORY_IDS.has(cat.id))

/** Layout: about/TechStack UI — integration categories (does not change About or TechStackShared). */
const TechStack = () => {
  const defaultCategory =
    modernizationTechCategories.find((c) => c.id === 'backend') ?? modernizationTechCategories[0]
  const [activeId, setActiveId] = useState(defaultCategory.id)
  const active = modernizationTechCategories.find((c) => c.id === activeId) ?? defaultCategory
  const tabsId = useId()
      ...
      <div className="container relative">
        ...
        >
          ...
          <p className="mt-5 text-base leading-relaxed text-[#808080] transition-colors duration-300 sm:text-lg">
            Proven tools we use to modernize, connect, and extend the systems your business already depends on.
          </p>
        </motion.div>
        ...
        <LayoutGroup id={tabsId}>
          <div
            role="tablist"
            aria-label="Modernization and integration technology categories"
          ...
          >
            {modernizationTechCategories.map((cat) => {
