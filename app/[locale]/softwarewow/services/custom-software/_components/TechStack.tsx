...
/** Keep custom-software categories for this service page only. */
const HIDDEN_CATEGORY_IDS = new Set(['design', 'marketing', 'commerce', 'communication'])

const customSoftwareTechCategories = techCategories.filter((cat) => !HIDDEN_CATEGORY_IDS.has(cat.id))

/** Layout: about/TechStack UI — custom software categories (does not change About or TechStackShared). */
const TechStack = () => {
  const defaultCategory = customSoftwareTechCategories.find((c) => c.id === 'frontend') ?? customSoftwareTechCategories[0]
  const [activeId, setActiveId] = useState(defaultCategory.id)
  const active = customSoftwareTechCategories.find((c) => c.id === activeId) ?? defaultCategory
  const tabsId = useId()
      ...
      <div className="container relative">
        ...
        >
          ...
          <p className="mt-5 text-base leading-relaxed text-[#808080] transition-colors duration-300 sm:text-lg">
            Proven tools we use to design, build, and scale custom software around how your business works.
          </p>
        </motion.div>
        ...
        <LayoutGroup id={tabsId}>
          <div
            role="tablist"
            aria-label="Custom software technology categories"
          ...
          >
            {customSoftwareTechCategories.map((cat) => {
