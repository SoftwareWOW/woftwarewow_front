...
const DEFAULT_GALLERYDATA = [
  ...
  {
    id: 2,
    image: '/images/career-img/career-img-2.png',
  ...
  {
    id: 3,
    image: '/images/career-img/career-img-3.png',
  ...
  {
    id: 4,
    image: '/images/career-img/career-img-4.png',
...
const GalleryImages = ({ images }: GalleryImagesProps = {}) => {
  const galleryItems = mergeGalleryItems(DEFAULT_GALLERYDATA, images)
        ...
        <div className="relative grid grid-cols-1 max-lg:gap-6 md:grid-cols-2 lg:grid-cols-4">
          {galleryItems.map((image) => (
            <figure key={image.id} className={`w-full ${image.order}`}>
              <img src={image.image} alt={image.alt} className={image.className} />
            </figure>
          ))}
        </div>
      </div>
