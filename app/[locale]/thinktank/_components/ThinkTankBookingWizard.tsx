...
type ThinkTankBookingWizardProps = {
  calLink?: string
}
...
export default function ThinkTankBookingWizard({ calLink }: ThinkTankBookingWizardProps) {
  ...
  const fetchSlots = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/cal/thinktank/slots?${params.toString()}`)
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    setSubmitting(true)
    try {
      const response = await fetch('/api/cal/thinktank/book', {
        method: 'POST',
      ...
      setStep('success')
      toast?.showToast('Your Think Tank session has been booked successfully!')
    } catch (error) {
  ...
  const sidebarContent =
    step === 'details'
      ? {
          ...
          description: 'Please share your details and the project or initiative you want to discuss.',
        }
      : {
          icon: CalendarDays,
          title: 'Select Date & Time',
          description: 'Please select a date and time for your Think Tank session.',
        }
  ...
  if (step === 'success') {
    return (
      <div className="flex min-h-[520px] flex-col items-center justify-center rounded-radius-md border border-black/10 bg-backgroundBody px-6 py-16 text-center dark:border-white/10 dark:bg-dark">
        ...
        <h3 className="text-2xl text-secondary dark:text-backgroundBody">Session confirmed</h3>
        <p className="mt-3 max-w-md text-base leading-relaxed text-muted">
          Thank you, {firstName}! Your Think Tank session is scheduled
        ...
        <div className="flex min-w-0 flex-col bg-backgroundBody dark:bg-dark">
          <div className="border-b border-black/5 px-5 py-4 dark:border-white/5 sm:px-8">
            <h4 className="text-lg text-secondary dark:text-backgroundBody md:text-xl">
              {step === 'details' ? 'Your Information' : 'Date & Time Selection'}
            </h4>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-5 py-6 sm:px-8">
            ...
            {step === 'details' && (
              <form id="thinktank-booking-form" onSubmit={handleSubmit} className="space-y-5">
                ...
                <textarea
                  ...
                  onChange={(e) => setProjectInitiative(e.target.value)}
                  rows={5}
                ...
                {selectedDate && selectedSlot && (
                  <p className="text-sm text-muted">
                    Selected: {formatDisplayDate(selectedDate)} at {formatTime(selectedSlot, timeZone)} ({timeZone})
                  </p>
                )}
              </form>
            )}
