...
const BlogItemsCarousel: FC<BlogItemsCarouselProps> = ({
  items,
  hrefPrefix,
  buttonText,
  showHeader = true,
  ...
  const uniqueItems = useMemo(() => {
    ...
    const deduped: BlogCarouselItem[] = []
    ...
    for (const item of items) {
      ...
      seen.add(key)
      deduped.push(item)
      if (typeof maxItems === 'number' && deduped.length >= maxItems) break
    }
  ...
  }, [api, isAutoPlay, uniqueItems.length])

  useEffect(() => {
    if (!api) return

    const onSelect = () => {
      const snapCount = uniqueItems.length
      if (!snapCount) return
      setCurrentIndex(api.selectedScrollSnap() % snapCount)
    }
      ...
      <div className="relative z-10 mx-auto max-w-[1320px]">
        {showHeader && (
          <RevealWrapper className="mb-10 text-center md:mb-14">
            <SectionLabel className="mx-auto mb-5">{sectionLabel}</SectionLabel>
          ...
          >
            <CarouselContent className="-ml-4 md:-ml-6">
              {uniqueItems.map((item, index) => {
                ...
                const href = `${hrefPrefix}${item.slug}`
                  ...
                  >
                    <article className="group flex h-full flex-col">
                      ...
                      <div className="mt-auto">
                        <ButtonComponent href={href} variant={isFeatured ? 'primary' : 'white'}>
                          {buttonText}
                        </ButtonComponent>
                      </div>
                    </article>
...
export default BlogItemsCarousel
