import { CAL_BOOK_API_VERSION, getThinkTankCalComConfig, getValidCalApiKey } from '@/lib/calcom/config'
...
export async function POST(request: Request) {
  ...
  const projectInitiative = body.projectInitiative?.trim() ?? ''
  const timeZone = body.timeZone?.trim() || 'America/Toronto'

  if (!firstName || !lastName || !email || !start || !projectInitiative) {
    return NextResponse.json(
      { error: 'First name, last name, email, project or initiative, and time slot are required.' },
      { status: 400 },
    )
  }
  ...
  const bookingPayload = {
    ...
    metadata: { notes: `Project / Initiative: ${projectInitiative}` },
  }
