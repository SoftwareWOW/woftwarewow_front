...
download(companyUrls.imgGroup29 ?? companyUrls.imgGroup29, join(root, 'public/images/wow/nav/checkmark.png'))
download(companyUrls.imgArrowLongRight, join(root, 'public/images/wow/nav/arrow-right.png'))

const output = `/** Auto-generated by scripts/download-nav-assets.mjs — do not edit manually */
export const navIconAssets: Record<string, string | string[]> = ${JSON.stringify(iconAssets, null, 2)}

export const navDivisionAssets: Record<string, string[]> = ${JSON.stringify(divisionAssets, null, 2)}
...
writeFileSync(join(root, 'components/wow/nav/nav-assets.ts'), output, 'utf8')
console.log(`Icons: ${Object.keys(iconAssets).length}, Divisions: ${Object.keys(divisionAssets).length}`)
